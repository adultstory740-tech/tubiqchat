    ...
    <div className="max-w-4xl mx-auto flex flex-col items-center">
      ...
      <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-6 mb-12 text-center text-pink-100/90 text-sm sm:text-base font-medium">
        ...
        <p className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4">
          <span>⚠️ Profiles are virtual personalities</span>
          <span className="hidden sm:inline text-white/30">•</span>
          <span>Service is for entertainment & conversation only</span>
          <span className="hidden sm:inline text-white/30">•</span>
          <span>No real-life meetings or relationships</span>
        </p>
      </div>

      {/* Links */}
      <div className="flex flex-wrap justify-center gap-6 sm:gap-10 text-pink-200/70 text-sm mb-10 font-medium">
        <a href="mailto:support@loverchat.in" className="hover:text-white transition-colors">About Us</a>
        <a href="mailto:support@loverchat.in" className="hover:text-white transition-colors">Contact Us</a>
        <a href="/privacy" className="hover:text-white transition-colors">Privacy Policy</a>
        <a href="/terms" className="hover:text-white transition-colors">Terms & Conditions</a>
        <a href="/refund" className="hover:text-white transition-colors">Refund Policy</a>
      </div>

      <p className="text-pink-100 mb-6 font-semibold">Email: support@loverchat.in</p>

      <p className="text-base tracking-wide text-gray-500">
        © 2026 LoverChat • 18+ Only
      </p>
    </div>
  </footer>
);
